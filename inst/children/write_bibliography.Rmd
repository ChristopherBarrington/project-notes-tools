...
RefManageR::GetBibEntryWithDOI(doi=bibliography_doi) %>%
  RefManageR::WriteBib(bib=., file=file.path(project.notes.tools:::project_paths$knitting_path(), 'scratch', 'bibliography.bib'), append=FALSE)

# create the publications content
sprintf(fmt='cd %s ; academic import --bibtex %s/scratch/bibliography.bib --publication-dir publication --no-overwrite', website_path, project.notes.tools:::project_paths$knitting_path()) %>%
...
file.path(project.notes.tools:::project_paths$knitting_path(), 'scratch', 'bibliography.bib') %>%
